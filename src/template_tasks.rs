    ...
    pub fn build_template<'a>(&'a self, entry_date: NaiveDate) -> Element<'a, TemplateTaskMessage> {
        ...
        if let Some(entry) = self.entries.get(&entry_date) {
            match entry {
                ...
                TaskDataFormat::DualBinary(dual_binary_data) => {
                    let check_first =
                        checkbox("", dual_binary_data.completed_first).on_toggle(move |ticked| {
                            TemplateTaskMessage::snapshot(
                                self,
                                entry_date,
                                TemplateMessage::DualBinary(DualBinaryMessage::CheckedFirst(
                                    ticked,
                                )),
                            )
                        });
                    let check_second =
                        checkbox("", dual_binary_data.completed_second).on_toggle(move |ticked| {
                            TemplateTaskMessage::snapshot(
                                self,
                                entry_date,
                                TemplateMessage::DualBinary(DualBinaryMessage::CheckedSecond(
                                    ticked,
                                )),
                            )
                        });
                    ...
                    let text = widget::text_editor(&dual_binary_data.text_content).on_action(
                        move |action| {
                            TemplateTaskMessage::snapshot(
                                self,
                                entry_date,
                                TemplateMessage::DualBinary(DualBinaryMessage::TextEdit(action)),
                            )
                        },
                    );
    ...
    pub fn update(&mut self, task_message: TemplateTaskMessage) {
        if let Some(data_format) = self.get_entry_mut(task_message.date) {
            match (task_message.message, task_message.task_type, data_format) {
                ...
                (
                    TemplateMessage::DualBinary(dual_binary_message),
                    TaskType::DualBinary,
                    TaskDataFormat::DualBinary(dual_binary_data),
                ) => match dual_binary_message {
                    DualBinaryMessage::CheckedFirst(checked) => {
                        dual_binary_data.completed_first = checked;
                    }
                    DualBinaryMessage::CheckedSecond(checked) => {
                        dual_binary_data.completed_second = checked;
                    }
                    DualBinaryMessage::TextEdit(action) => {
                        dual_binary_data.text_content.perform(action);
                    }
                },
                _ => {
